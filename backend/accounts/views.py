...
class IsSuperAdmin(permissions.BasePermission):
    def has_permission(self, request, view):
        ...
        return bool(u and u.is_authenticated and (
            u.is_superuser or u.role in (Role.SUPER_ADMIN, Role.CEO, Role.GOLDBOD_OFFICER)))
...
class AdminUserViewSet(viewsets.ModelViewSet):
    ...
    def perform_update(self, serializer):
        ...
        if user.position_id and user.position and user.role != user.position.base_role:
            user.role = user.position.base_role
...
class PositionViewSet(viewsets.ModelViewSet):
    """Admin-defined positions (Deputy CEO, Director, ...) inheriting a base role."""
    from .models import Position as _P
    queryset = _P.objects.all().order_by("title")
