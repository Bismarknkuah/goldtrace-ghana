...
class Role(models.TextChoices):
    ...
    INDEPENDENT_AUDITOR = "independent_auditor", "Independent Auditor"
...
AUDITOR_ROLES = {Role.INDEPENDENT_AUDITOR}


class Position(models.Model):
    """An admin-defined title (e.g. "Deputy CEO", "Director of Licensing") that
    inherits its permissions, scoping and navigation from a built-in base role."""
    title = models.CharField(max_length=80, unique=True)
    base_role = models.CharField(max_length=32, choices=Role.choices)
    description = models.CharField(max_length=200, blank=True)
    active = models.BooleanField(default=True)
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"{self.title} ({self.get_base_role_display()})"
...
class User(AbstractUser):
    ...
    def __str__(self):
        return f"{self.username} ({self.display_title})"

    @property
    def display_title(self) -> str:
        return self.position.title if self.position_id and self.position else self.get_role_display()
