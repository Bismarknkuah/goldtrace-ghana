...
class GoldBatchViewSet(viewsets.ModelViewSet):
    ...
    def track(self, request):
        ...
        return Response({
            ...
            "lot_risk": compute_lot_risk(batch, chain),
        })

    @extend_schema(tags=["production"], responses=OpenApiTypes.OBJECT)
    @action(detail=True, methods=["get"])
    def graph(self, request, pk=None):
        """Relationship graph for a gold lot: every actor and record connected to
        it, staged source -> assay -> ownership -> custody -> refinery -> export."""
        batch = self.get_object()
        nodes = []
        edges = []

        def node(nid, label, ntype, stage, detail=""):
        ...
        # Concession + miner (source)
        if batch.concession_id:
            node(f"con-{batch.concession_id}", batch.concession.reference,
                 "concession", "source", "Licensed concession")
        node(f"miner-{batch.miner_id}", batch.miner.license_number, "miner", "source",
             f"{batch.miner.region or ''}")
        if batch.concession_id:
            edge(f"con-{batch.concession_id}", f"miner-{batch.miner_id}", "operated by")

        # The lot itself
        lot = f"lot-{batch.id}"
        node(lot, batch.batch_code, "lot", "lot",
             f"{batch.gross_weight_g} g @ {batch.fineness or '?'}‰")
        edge(f"miner-{batch.miner_id}", lot, "produced")

        # Assay
        assay = getattr(batch, "assay", None)
        if assay:
            aid = f"assay-{assay.id}"
            who = assay.assayer.username if assay.assayer_id else "assayer"
            node(aid, f"Assay · {who}", "assay", "assay",
                 f"{assay.fineness or ''}‰" if hasattr(assay, "fineness") else "")
            edge(lot, aid, "verified by")

        # Ownership chain (transfers)
        from trading.models import OwnershipTransfer
        for tr in OwnershipTransfer.objects.filter(batch=batch).select_related(
                "seller", "buyer").order_by("created_at"):
            if tr.buyer_id:
                bid = f"owner-{tr.buyer_id}"
                if not any(n["id"] == bid for n in nodes):
                    node(bid, tr.buyer.username, "owner", "ownership",
                         tr.buyer.get_role_display())
                seller = f"owner-{tr.seller_id}" if tr.seller_id else lot
                edge(seller if any(n["id"] == seller for n in nodes) else lot, bid,
                     "sold to" + (" (irregular)" if tr.irregular else ""))

        # Custody / transport
        delivery = batch.deliveries.order_by("-created_at").first() if hasattr(batch, "deliveries") else None
        if delivery:
            did = f"deliv-{delivery.id}"
            node(did, "Secure transport", "custody", "custody",
                 getattr(delivery, "seal_number", "") or "")
            edge(lot, did, "in custody")

        # Export + off-taker
        cert = batch.export_certificates.order_by("-created_at").first() if hasattr(batch, "export_certificates") else None
        if cert:
            cid = f"cert-{cert.id}"
            node(cid, f"Export · {cert.destination_country}", "export", "export",
                 cert.get_status_display())
            edge(lot, cid, "exported as")

        stages = ["source", "lot", "assay", "ownership", "custody", "refinery", "export"]
        return Response({"batch_code": batch.batch_code, "stages": stages,
                         "nodes": nodes, "edges": edges})

    @extend_schema(tags=["production"], responses=OpenApiTypes.OBJECT)
    @action(detail=True, methods=["get"])
    def compliance_check(self, request, pk=None):
        """Preview the pre-transaction compliance gate for the current user."""
        return Response(precheck_transaction(self.get_object(), request.user))

    @extend_schema(tags=["production"], responses=OpenApiTypes.OBJECT)
    @action(detail=False, methods=["post"])
    def aggregate(self, request):
        """Combine several owned lots into one aggregated lot, preserving ancestry."""
        from decimal import Decimal

        ids = request.data.get("batch_ids", [])
        if len(ids) < 2:
            return Response({"detail": "Select at least two lots to aggregate."}, status=400)
        lots = list(GoldBatch.objects.filter(id__in=ids))
        if any(l.current_owner_id != request.user.id and not request.user.is_superuser for l in lots):
            return Response({"detail": "You can only aggregate lots you own."}, status=403)
        gross = sum(Decimal(str(l.gross_weight_g or 0)) for l in lots)
        agg = GoldBatch.objects.create(
            miner=lots[0].miner, created_by=request.user, current_owner=request.user,
            gross_weight_g=gross, is_aggregated=True,
            source_lots=[l.batch_code for l in lots])
        append_custody_event(agg, CustodyEventType.TRANSFER, actor=request.user,
                             from_party="aggregation", to_party=str(request.user))
        return Response({"detail": f"Aggregated {len(lots)} lots into {agg.batch_code}.",
                         "batch_code": agg.batch_code, "source_lots": agg.source_lots,
                         "gross_weight_g": str(gross)})

    @extend_schema(tags=["production"], responses=OpenApiTypes.OBJECT)
    @action(detail=True, methods=["post"])
    def refine(self, request, pk=None):
        """Refinery mass-balance reconciliation: input = output + loss + sample + residue.
        A mismatch beyond tolerance raises a mass-balance anomaly."""
        from decimal import Decimal
        ...
        inp, out = dv("input_g"), dv("output_g")
        loss, sample, residue = dv("loss_g"), dv("sample_g"), dv("residue_g")
        accounted = out + loss + sample + residue
        discrepancy = inp - accounted
        from core.models import SystemConfig
        tolerance = inp * Decimal(str(SystemConfig.get().mass_balance_tolerance_pct)) / Decimal("100")
        ok = abs(discrepancy) <= tolerance
        batch.refine_input_g, batch.refine_output_g = inp, out
        batch.refine_loss_g, batch.refine_sample_g, batch.refine_residue_g = loss, sample, residue
        batch.mass_balance_ok = ok
        batch.status = BatchStatus.REFINED if hasattr(BatchStatus, "REFINED") else batch.status
        batch.save()
        if not ok:
            from security.models import IncidentStatus, IncidentType, SecurityIncident
            SecurityIncident.objects.create(
                batch=batch, incident_type=IncidentType.UNACCOUNTED, status=IncidentStatus.OPEN,
                note=f"Mass-balance anomaly: {discrepancy} g unaccounted (input {inp} vs {accounted}).",
                reported_by=request.user)
            batch.security_status = SecurityStatus.FLAGGED
            batch.save(update_fields=["security_status"])
        return Response({"mass_balance_ok": ok, "discrepancy_g": str(discrepancy),
                         "accounted_g": str(accounted), "input_g": str(inp),
                         "message": "Reconciled." if ok else
                         "MASS-BALANCE ANOMALY — a fraud case has been raised."})
    ...
    def buy(self, request, pk=None):
        ...
        # Pre-transaction compliance gate.
        gate = precheck_transaction(batch, buyer)
        if gate["decision"] == "block":
            return Response({"detail": "Purchase blocked by compliance checks.",
                             "compliance": gate}, status=403)
        needs_review = gate["decision"] == "review"
        ...
        OwnershipTransfer.objects.create(
            ...
            stage=stage, irregular=(not eligible) or needs_review, status=TransferStatus.PENDING)
        msg = ("Purchase request submitted for HUMAN REVIEW (compliance flags raised); "
               "a GoldBod officer must approve it." if needs_review else
               "Purchase request submitted. The seller (or a GoldBod officer) must approve it "
               "before ownership moves and a receipt is issued.")
        return Response({"detail": msg, "batch_code": batch.batch_code, "compliance": gate})
