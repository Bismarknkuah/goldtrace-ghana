...
class GoldBatch(TimeStampedModel):
    ...
    # Aggregation ancestry: batch codes of the lots merged into this one.
    source_lots = models.JSONField(default=list, blank=True)
    is_aggregated = models.BooleanField(default=False)
    # Refinery mass-balance reconciliation.
    refine_input_g = models.DecimalField(max_digits=16, decimal_places=3, null=True, blank=True)
    refine_output_g = models.DecimalField(max_digits=16, decimal_places=3, null=True, blank=True)
    refine_loss_g = models.DecimalField(max_digits=16, decimal_places=3, null=True, blank=True)
    refine_sample_g = models.DecimalField(max_digits=16, decimal_places=3, null=True, blank=True)
    refine_residue_g = models.DecimalField(max_digits=16, decimal_places=3, null=True, blank=True)
    mass_balance_ok = models.BooleanField(null=True, blank=True)
