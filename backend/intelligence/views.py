...
from accounts.models import AUDITOR_ROLES, ENFORCEMENT_ROLES, REGULATOR_ROLES, Role
...
@extend_schema(tags=["intelligence"], responses=OpenApiTypes.OBJECT)
class ComplianceMapView(APIView):
    """Live compliance status per operator for top management: green (compliant),
    amber (warning), red (violating) - plotted on the national map."""
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request):
        ...
        from licensing.models import License, LicenseStatus
        from security.models import IncidentStatus, SecurityIncident
        from trading.models import OwnershipTransfer
        User = get_user_model()

        # Gather violation/warning signals per operator.
        reasons = defaultdict(list)
        levels = defaultdict(lambda: 0)  # 0 compliant, 1 warning, 2 violating

        def flag(uid, level, reason):
            if uid is None:
                return
            reasons[uid].append(reason)
            levels[uid] = max(levels[uid], level)

        for lic in License.objects.select_related("holder"):
            if lic.status in (LicenseStatus.SUSPENDED, LicenseStatus.REVOKED):
                flag(lic.holder_id, 2, f"Licence {lic.get_status_display().lower()}")
            elif lic.status == LicenseStatus.EXPIRED or (
                    lic.status == LicenseStatus.ACTIVE and not lic.is_valid):
                flag(lic.holder_id, 1, "Licence expired")
            elif lic.status == LicenseStatus.PENDING:
                flag(lic.holder_id, 1, "Licence pending approval")

        open_inc = (SecurityIncident.objects
                    .exclude(status__in=[IncidentStatus.RECOVERED, IncidentStatus.CLOSED])
                    .select_related("batch__current_owner"))
        for inc in open_inc:
            owner = getattr(inc.batch, "current_owner", None)
            if owner:
                flag(owner.id, 2, f"Open {inc.get_incident_type_display().lower()} incident")

        for tr in OwnershipTransfer.objects.filter(irregular=True).select_related("seller"):
            if tr.seller_id:
                flag(tr.seller_id, 1, "Irregular (out-of-flow) transfer")

        operators = []
        summary = {"compliant": 0, "warning": 0, "violating": 0}
        for u in (User.objects.exclude(latitude=None).exclude(longitude=None)):
            lvl = levels.get(u.id, 0)
            status = ["compliant", "warning", "violating"][lvl]
            summary[status] += 1
            operators.append({
                "username": u.username, "role_display": u.get_role_display(),
                "region": u.region, "district": u.district,
                "latitude": u.latitude, "longitude": u.longitude,
                "status": status, "reasons": reasons.get(u.id, []),
            })
        return Response({"summary": summary, "operators": operators})


@extend_schema(tags=["intelligence"], responses=OpenApiTypes.OBJECT)
class RecommendationsView(APIView):
    """AI orchestrator: specialist agents -> Recommendation agent -> human decision."""
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request):
        ...
        # Reuse anomaly detection for fraud/risk signals.
        transfers = [{
            "seller_id": t.seller_id, "seller_name": t.seller.username if t.seller_id else None,
            "buyer_id": t.buyer_id, "buyer_name": t.buyer.username if t.buyer_id else None,
            "batch_code": t.batch.batch_code if t.batch_id else None,
            "created_at": t.created_at, "irregular": bool(getattr(t, "irregular", False)),
        } for t in OwnershipTransfer.objects.select_related("seller", "buyer", "batch")[:600]]

        from collections import defaultdict
        gross = defaultdict(float)
        batch_chains = []
        hotspot_regions = set()
        for b in GoldBatch.objects.select_related("miner")[:400]:
            owners = [e.metadata.get("to_party") for e in b.custody_events.order_by("created_at")
                      if isinstance(e.metadata, dict) and e.metadata.get("to_party")]
            batch_chains.append({"batch_code": b.batch_code, "owner_sequence": owners,
                                 "chain_valid": verify_chain(b)["valid"]})
            if b.miner and b.miner.user_id:
                gross[b.miner.user_id] += float(b.gross_weight_g or 0)
        participants = [{"id": u.id, "username": u.username, "role": u.role,
                         "phone": getattr(u, "phone", ""), "organization": getattr(u, "organization", ""),
                         "total_gross_g": gross.get(u.id, 0)} for u in User.objects.all()[:800]]
        anomalies = detect_anomalies(transfers=transfers, participants=participants,
                                     batch_chains=batch_chains)

        licences = [{"holder": l.holder.username if l.holder_id else "?", "status": l.status}
                    for l in License.objects.select_related("holder")[:400]]

        result = orchestrate_agents(
            anomalies=anomalies, licences=licences, transfers=transfers,
            hotspot_regions=list(hotspot_regions), stuck_deliveries=0)
        brief = maybe_llm_summary(result["recommendations"])
        if brief:
