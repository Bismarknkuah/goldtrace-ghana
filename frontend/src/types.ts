...
export interface AdminUser {
  ...
  role: Role; role_display: string; position?: string | null; position_title?: string | null;
...
export interface LotRisk {
  origin: number; transaction: number; assay: number; custody: number;
  environmental: number; export: number; overall: number;
...
export interface ComplianceOperator {
  username: string; role_display: string; region: string; district: string;
  latitude: number; longitude: number;
  status: "compliant" | "warning" | "violating"; reasons: string[];
}
export interface ComplianceMap {
  summary: { compliant: number; warning: number; violating: number };
  operators: ComplianceOperator[];
}

export interface GraphNode {
  id: string; label: string; type: string; stage: string; detail: string;
}
export interface GraphEdge { from: string; to: string; label: string; }
export interface BatchGraph {
  batch_code: string; stages: string[]; nodes: GraphNode[]; edges: GraphEdge[];
}

export interface Recommendation {
  agent: string; severity: "critical" | "high" | "medium" | "low";
  finding: string; detail: string; recommended_action: string; requires_human: boolean;
}
export interface Recommendations {
  agent_findings: Record<string, number>; recommendations: Recommendation[];
  total: number; executive_brief?: string;
}

export interface FxReconciliation {
  summary: { total: number; flagged: number };
  rows: { certificate: string; destination: string; exporter: string | null;
    fine_weight_g: string; expected_usd: string; declared_usd: string | null;
    status: "reconciled" | "flagged" | "missing" }[];
}
export interface EnvRisk {
  regions: { region: string; hotspots: number; active_sites: number;
    risk_score: number; band: "low" | "medium" | "high" | "critical" }[];
}

export interface Detection {
  id: string; latitude: number; longitude: number; region: string; source: string;
  confidence: number; area_ha: number | null; detected_at: string; operator_name: string;
  status: string; status_display: string; matched_concession: string | null;
  concession_ref: string | null; note: string;
}

export interface IllegalAlert {
  latitude: number; longitude: number; region: string; source: string;
  probability: number; band: "low" | "medium" | "high" | "critical";
  drivers: string[]; status: string | null; maps_link: string;
}
export interface IllegalAlerts {
  summary: { total: number; critical: number; high: number }; alerts: IllegalAlert[];
}

export interface ReportLink { id: string; token: string; title: string; active: boolean; submissions: number; created_at: string; }
export interface PublicReport {
  id: string; category: string; category_display: string; description: string;
  reporter_name: string; reporter_phone: string; latitude: number | null; longitude: number | null;
  region: string; location_text: string; attachments: { name: string; type: string; data: string }[];
  attachment_count: number; status: string; status_display: string; assigned_to: string[];
  assigned_names: string[]; detection: string | null; officer_note: string; created_at: string;
}

export interface Position { id: string; title: string; base_role: string; base_role_display: string;
  description: string; active: boolean; user_count: number; created_at: string; }
export interface SystemConfig {
  org_name: string; tagline: string; contact_email: string; report_form_title: string;
  risk_block_threshold: number; risk_review_threshold: number; mass_balance_tolerance_pct: number;
  fx_discrepancy_pct: number; max_report_attachments: number;
}
