          ...
          {data.lot_risk && (
            <Card><CardContent>
              <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 1 }}>
                <Typography variant="h6">Gold-lot risk profile</Typography>
                <Chip label={`${data.lot_risk.overall}/100 · ${data.lot_risk.band.toUpperCase()}`}
                  color={data.lot_risk.band === "low" ? "success"
                    : data.lot_risk.band === "medium" ? "warning" : "error"} />
              </Stack>
              <Box className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {([["Origin","origin"],["Transaction","transaction"],["Assay","assay"],
                   ["Custody","custody"],["Environmental","environmental"],["Export","export"]] as const)
                  .map(([label, key]) => {
                    const v = data.lot_risk![key];
                    const color = v >= 70 ? "#B23A2E" : v >= 45 ? "#D9822B" : v >= 25 ? "#C9A227" : "#2E7D32";
                    return (
                      <Box key={key}>
                        <Stack direction="row" justifyContent="space-between">
                          <Typography variant="body2" color="text.secondary">{label}</Typography>
                          <Typography variant="body2" sx={{ fontWeight: 600, color }}>{v}</Typography>
                        </Stack>
                        <Box sx={{ height: 6, borderRadius: 3, bgcolor: "#EEE", mt: 0.5 }}>
                          <Box sx={{ height: 6, borderRadius: 3, width: `${v}%`, bgcolor: color }} />
                        </Box>
                      </Box>
                    );
                  })}
