...
import { useRiskOverviewQuery, useParticipantRiskQuery, useAnomaliesQuery,
  useRecommendationsQuery } from "../services/api";
      ...
      {recs && recs.recommendations.length > 0 && (
        <Card sx={{ borderLeft: "4px solid #C9A227" }}><CardContent>
          <Typography variant="h6" gutterBottom>
            AI orchestrator · {recs.total} recommendation{recs.total === 1 ? "" : "s"} for review
          </Typography>
          {recs.executive_brief && (
            <Typography variant="body2" sx={{ mb: 2, fontStyle: "italic" }}>{recs.executive_brief}</Typography>
          )}
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            Fraud, Compliance, Risk, Sustainability and Logistics agents feed a Recommendation agent —
            every action requires a human decision.
          </Typography>
          <Stack spacing={1.25}>
            {recs.recommendations.slice(0, 10).map((r: import("../types").Recommendation, i: number) => (
              <Box key={i} sx={{ display: "flex", gap: 1.5, alignItems: "flex-start",
                borderLeft: `3px solid ${r.severity === "critical" ? "#8E1B12" : r.severity === "high" ? "#B23A2E" : "#C9A227"}`, pl: 1.5 }}>
                <Chip size="small" label={r.agent}
                  color={r.severity === "critical" || r.severity === "high" ? "error" : "warning"} />
                <Box>
                  <Typography sx={{ fontWeight: 600 }}>{r.finding}</Typography>
                  <Typography variant="body2" color="text.secondary">{r.detail}</Typography>
                  <Typography variant="body2" sx={{ color: "#7A6A2E" }}>→ {r.recommended_action}</Typography>
                </Box>
              </Box>
            ))}
          </Stack>
