import { Box, Card, CardContent, Chip, Stack, Table, TableBody, TableCell, TableHead,
  TableRow, Typography } from "@mui/material";
import type { FxReconciliation, EnvRisk } from "../types";
import { Bar, BarChart, CartesianGrid, ResponsiveContainer, Tooltip, XAxis, YAxis } from "recharts";
import { useState } from "react";
import { Button, TextField } from "@mui/material";
import { useRevenueOverviewQuery, useCurrentRateQuery, useCreateRateMutation, useFxReconciliationQuery, useEnvironmentalRiskQuery } from "../services/api";
...
export default function Revenue() {
  const { data: fx } = useFxReconciliationQuery();
  ...
  return (
    <Stack spacing={3}>
      ...
      <Card><CardContent>
        ...
        <Card><CardContent>
          <Typography variant="h6" gutterBottom>
            BoG FX reconciliation · {fx.summary.flagged} of {fx.summary.total} need review
          </Typography>
          <Table size="small">
            <TableHead><TableRow>
              <TableCell>Certificate</TableCell><TableCell>Destination</TableCell>
              <TableCell>Expected (USD)</TableCell><TableCell>Declared (USD)</TableCell>
              <TableCell>Status</TableCell></TableRow></TableHead>
            <TableBody>
              {fx.rows.slice(0, 20).map((row: FxReconciliation["rows"][number]) => (
                <TableRow key={row.certificate}>
                  <TableCell className="mono">{row.certificate}</TableCell>
                  <TableCell>{row.destination}</TableCell>
                  <TableCell className="mono">{Number(row.expected_usd).toLocaleString()}</TableCell>
                  <TableCell className="mono">{row.declared_usd ? Number(row.declared_usd).toLocaleString() : "—"}</TableCell>
                  <TableCell><Chip size="small" label={row.status}
                    color={row.status === "reconciled" ? "success" : row.status === "flagged" ? "error" : "warning"} /></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
      ...
      {env && env.regions.length > 0 && (
        <Card><CardContent>
          <Typography variant="h6" gutterBottom>Environmental risk register</Typography>
          <Table size="small">
            <TableHead><TableRow>
              <TableCell>Region</TableCell><TableCell>Hotspots</TableCell>
              <TableCell>Active sites</TableCell><TableCell>Risk</TableCell></TableRow></TableHead>
            <TableBody>
              {env.regions.map((rg: EnvRisk["regions"][number]) => (
                <TableRow key={rg.region}>
                  <TableCell>{rg.region}</TableCell><TableCell>{rg.hotspots}</TableCell>
                  <TableCell>{rg.active_sites}</TableCell>
                  <TableCell><Chip size="small" label={`${rg.risk_score} · ${rg.band}`}
                    color={rg.band === "low" ? "success" : rg.band === "medium" ? "warning" : "error"} /></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
