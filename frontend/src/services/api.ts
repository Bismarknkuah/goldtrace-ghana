...
import type {
  MarketListing, AnomalyFeed, ComplianceMap, BatchGraph, Recommendations, FxReconciliation, EnvRisk, Detection, IllegalAlerts, ReportLink, PublicReport, Position, SystemConfig,
...
// On a 401, try to refresh the access token once and retry; if that fails,
// clear the session so the app returns to the login screen instead of a
// wall of 401 errors.
  ...
  if (result.error && result.error.status === 401) {
    ...
    if (refresh) {
      const r = await rawBaseQuery(
        ...
        apiCtx, extra);
      const data = r.data as { access?: string } | undefined;
      if (data?.access) {
        apiCtx.dispatch(setTokens({ access: data.access, refresh }));
        result = await rawBaseQuery(args, apiCtx, extra);  // retry original
      } else {
...
export const api = createApi({
  reducerPath: "api",
  baseQuery,
  tagTypes: ["Batch", "Miner", "Transfer", "Certificate", "Delivery", "Courier", "Receipt", "Security", "Company", "License", "Compliance", "Rate", "AdminUser", "Market", "Features", "Detection", "Report"],
  endpoints: (b) => ({
    ...
    createAdminUser: b.mutation<AdminUser, { username: string; password: string; role: string; position?: string | null;
    ...
    batchGraph: b.query<BatchGraph, string>({
      query: (id) => `/production/batches/${id}/graph/`,
    }),
    ...
    complianceMap: b.query<ComplianceMap, void>({ query: () => "/intelligence/compliance-map/" }),
    recommendations: b.query<Recommendations, void>({ query: () => "/intelligence/recommendations/" }),
    fxReconciliation: b.query<FxReconciliation, void>({ query: () => "/revenue/fx-reconciliation/" }),
    environmentalRisk: b.query<EnvRisk, void>({ query: () => "/gis/environmental-risk/" }),
    illegalMiningAlerts: b.query<IllegalAlerts, void>({ query: () => "/gis/illegal-mining-alerts/", providesTags: ["Detection"] }),
    reportLinks: b.query<Paginated<ReportLink>, void>({ query: () => "/reports/links/", providesTags: ["Report"] }),
    createReportLink: b.mutation<ReportLink, { title: string }>({
      query: (body) => ({ url: "/reports/links/", method: "POST", body }), invalidatesTags: ["Report"],
    }),
    publicReports: b.query<Paginated<PublicReport>, void>({ query: () => "/reports/", providesTags: ["Report"] }),
    assignReport: b.mutation<PublicReport, { id: string; user_ids: string[] }>({
      query: ({ id, user_ids }) => ({ url: `/reports/${id}/assign/`, method: "POST", body: { user_ids } }),
      invalidatesTags: ["Report"],
    }),
    setReportStatus: b.mutation<PublicReport, { id: string; status: string; note?: string }>({
      query: ({ id, ...body }) => ({ url: `/reports/${id}/set_status/`, method: "POST", body }),
      invalidatesTags: ["Report"],
    }),
    myFeatures: b.query<{ role: string; features: string[] | null }, void>({ query: () => "/core/features/", providesTags: ["Features"] }),
    allRoleFeatures: b.query<Record<string, string[]>, void>({ query: () => "/core/features/?all=1", providesTags: ["Features"] }),
    positions: b.query<Paginated<Position>, void>({ query: () => "/auth/admin/positions/", providesTags: ["AdminUser"] }),
    createPosition: b.mutation<Position, { title: string; base_role: string; description?: string }>({
      query: (body) => ({ url: "/auth/admin/positions/", method: "POST", body }), invalidatesTags: ["AdminUser"],
    }),
    deletePosition: b.mutation<unknown, string>({
      query: (id) => ({ url: `/auth/admin/positions/${id}/`, method: "DELETE" }), invalidatesTags: ["AdminUser"],
    }),
    systemConfig: b.query<SystemConfig, void>({ query: () => "/core/config/", providesTags: ["Features"] }),
    updateSystemConfig: b.mutation<SystemConfig, Partial<SystemConfig>>({
      query: (body) => ({ url: "/core/config/", method: "PUT", body }), invalidatesTags: ["Features"],
    }),
    setRoleFeatures: b.mutation<unknown, { role: string; features: string[] }>({
      query: (body) => ({ url: "/core/features/", method: "PUT", body }), invalidatesTags: ["Features"],
    }),
    detections: b.query<Paginated<Detection>, void>({ query: () => "/gis/detections/", providesTags: ["Detection"] }),
    detectionsGeo: b.query<FeatureCollection, void>({ query: () => "/gis/detections/geojson/", providesTags: ["Detection"] }),
    ingestDetection: b.mutation<Detection, Partial<Detection>>({
      query: (body) => ({ url: "/gis/detections/", method: "POST", body }), invalidatesTags: ["Detection"],
    }),
    setDetectionStatus: b.mutation<Detection, { id: string; status: string; note?: string }>({
      query: ({ id, ...body }) => ({ url: `/gis/detections/${id}/set_status/`, method: "POST", body }),
      invalidatesTags: ["Detection"],
    }),
    aggregateLots: b.mutation<{ detail: string; batch_code: string }, string[]>({
      query: (batch_ids) => ({ url: "/production/batches/aggregate/", method: "POST", body: { batch_ids } }),
      invalidatesTags: ["Batch", "Market"],
    }),
    refineBatch: b.mutation<{ mass_balance_ok: boolean; message: string; discrepancy_g: string }, { id: string; body: Record<string, string> }>({
      query: ({ id, body }) => ({ url: `/production/batches/${id}/refine/`, method: "POST", body }),
      invalidatesTags: ["Batch"],
    }),
...
export const {
  ...
  useMinersQuery,
  useBatchesQuery, useBatchQuery, useBatchGraphQuery, useCreateBatchMutation, useVerifyBatchQuery,
  ...
  useCurrentRateQuery, useCreateRateMutation, useParticipantRiskQuery, useOperatorsQuery, useComplianceMapQuery, useRecommendationsQuery,
  useAggregateLotsMutation, useRefineBatchMutation, useFxReconciliationQuery,
  useEnvironmentalRiskQuery, useIllegalMiningAlertsQuery, useReportLinksQuery, useCreateReportLinkMutation,
  usePublicReportsQuery, useAssignReportMutation, useSetReportStatusMutation, useMyFeaturesQuery, useAllRoleFeaturesQuery, useSetRoleFeaturesMutation,
  usePositionsQuery, useCreatePositionMutation, useDeletePositionMutation, useSystemConfigQuery, useUpdateSystemConfigMutation,
  useDetectionsQuery, useDetectionsGeoQuery, useIngestDetectionMutation, useSetDetectionStatusMutation,
  useTransparencyQuery, useMarketplaceQuery, useListForSaleMutation,
